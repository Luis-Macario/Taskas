...
BEGIN
    ...
    IF newBoardId = -1 then
        Raise warning ' The board id must be higher than 0';
    END IF;
    SELECT indexlist, lid, bid INTO oldIdx, oldList, oldBoard FROM cards where id = cid;
    IF oldIdx = -1 then
        Raise warning ' The card indexList must be higher than 0';
    END IF;
    ...
    IF (oldList != listId) then
        ...
        WHERE c.lid = listId and c.bid = newBoardId and c.indexlist >= cix;
        -- Updates the old list
        UPDATE cards as c
        SET indexlist = c.indexlist - 1
        WHERE c.lid = oldList and c.bid = oldBoard and oldIdx < c.indexlist;
    ELSE
        -- Same lists:
        if (oldIdx < cix) then
            UPDATE cards as c
